...
use janu_flow::model::{NodeKind, RegistryNode};

pub static ZF_OUTPUT_DIRECTORY: &str = "janu-flow";
...
pub struct CargoPkgMetadata {
    pub januflow: Option<CargoJanuFlow>,
}

#[derive(Clone, Debug, Deserialize, Default)]
#[serde(rename_all = "kebab-case", deny_unknown_fields)]
pub struct CargoJanuFlow {
    pub id: String,
    pub kind: NodeKind,
...
) -> CZFResult<(CargoJanuFlow, PathBuf, PathBuf)> {
    ...
    let metadata = toml::from_slice::<Cargo>(&content)?
        ...
        .ok_or_else(|| {
            CZFError::MissingField(
                ...
                "Missing package.metadata.januflow",
            )
        })?
        .januflow
        .ok_or_else(|| {
