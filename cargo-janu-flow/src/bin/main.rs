...
use janu_flow::model::node::{OperatorDescriptor, SinkDescriptor, SourceDescriptor};
use janu_flow::model::{NodeKind, RegistryNode, RegistryNodeArchitecture, RegistryNodeTag};
use janu_flow::NodeId;
...
use janu::*;
#[cfg(feature = "local_registry")]
use janu_flow_registry::RegistryClient;
#[cfg(feature = "local_registry")]
use janu_flow_registry::RegistryFileClient;
...
#[async_std::main]
async fn main() {
    // `cargo janu-flow` invocation passes the `janu-flow` arg through.
    ...
    #[cfg(feature = "local_registry")]
    {
        let mut zconfig = janu::config::Config::default();

        match zconfig.set_mode(Some("peer".parse())) {
            Ok => (),
            Err(e) => {
                println!(
                    "{}: to create Janu session configuration: {:?}",
                    "error".red().bold(),
                    e
                );
                exit(-1);
        ...
        let zsession = match janu::open(zconfig).await {
            Ok(zn) => Arc::new(zn),
            Err(e) => {
                println!("{}: to create Janu session: {:?}", "error".red().bold(), e);
                exit(-1);
            }
        };
    ...
    match args {
        ZFCtl::Build {
            ...
            if cargo_build_flags
                .first()
                .map_or(false, |arg| arg == "janu-flow")
            {
                cargo_build_flags.remove(0);
            }

            let (node_info, target_dir, manifest_dir) =
                match cargo_janu_flow::utils::from_manifest(&manifest_path, package) {
                    Ok(res) => res,
                    Err(_e) => {
            ...
            let (metadata_graph, _metadata_arch, descriptor) = match node_info.kind {
                NodeKind::Operator => {
                    if node_info.inputs.is_none() {
                        println!(
                            "{}: Janu-Flow metadata is missing inputs for Operator node",
                            "error".red().bold()
                        );
                        exit(-1);
                    }
                    ...
                    if node_info.outputs.is_none() {
                        println!(
                            "{}: Janu-Flow metadata is missing outputs for Operator node",
                            "error".red().bold()
                        );
                        exit(-1);
                    }
                    ...
                    if inputs.is_empty() {
                        println!("{}: Janu-Flow metadata has empty inputs for Operator, it should have at least one input", "error".red().bold());
                        exit(-1);
                    }

                    if outputs.is_empty() {
                        println!("{}: Janu-Flow metadata has empty outputs for Operator, it should have at least one output", "error".red().bold());
                        exit(-1);
                    }
                ...
                NodeKind::Source => {
                    if node_info.inputs.is_some() {
                        println!("{}: Janu-Flow metadata has inputs for Source node, they will be discarded", "warning".yellow().bold());
                    }

                    if node_info.outputs.is_none() {
                        println!(
                            "{}: Janu-Flow metadata is missing outputs for Source node",
                            "error".red().bold()
                        );
                        exit(-1);
                    }
                    ...
                    if outputs.is_empty() {
                        println!("{}: Janu-Flow metadata has empty outputs for Source, it should exactly one output", "error".red().bold());
                        exit(-1);
                    }

                    if outputs.len() > 1 {
                        println!("{}: Janu-Flow metadata has more than one output for Source, it should exactly one output", "error".red().bold());
                        exit(-1);
                    }
                ...
                NodeKind::Sink => {
                    if node_info.inputs.is_none() {
                        println!(
                            "{}: Janu-Flow metadata is missing inputs for Sink node",
                            "error".red().bold()
                        );
                        exit(-1);
                    }
                    ...
                    if node_info.outputs.is_some() {
                        println!("{}: Janu-Flow metadata has outputs for Sink node, they will be discarded", "warning".yellow().bold());
                    }

                    let inputs = node_info.inputs.unwrap();

                    if inputs.is_empty() {
                        println!("{}: Janu-Flow metadata has empty inputs for Sink, it should exactly one inputs", "error".red().bold());
                        exit(-1);
                    }

                    if inputs.len() > 1 {
                        println!("{}: Janu-Flow metadata has more than one input for Sink, it should exactly one input", "error".red().bold());
                        exit(-1);
                    }
            ...
            match cargo_janu_flow::utils::cargo_build(&cargo_build_flags, release, &manifest_dir) {
                Ok(_) => (),
                Err(_) => {
            ...
            match cargo_janu_flow::utils::store_zf_metadata(&metadata_graph, &target_dir) {
                Ok(res) => {
            ...
            match cargo_janu_flow::utils::store_zf_descriptor(
                &descriptor,
                &target_dir,
                &node_info.id,
            ) {
        ...
        ZFCtl::New { name, kind } => {
            match cargo_janu_flow::utils::create_crate(&name, kind.clone()).await {
                Ok(_) => {
                    println!(
                        "{} boilerplate for {} {} ",
                        "Created".green().bold(),
