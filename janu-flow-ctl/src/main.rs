...
async fn main() {
    env_logger::init();
    log::debug!("TAW Janu-Flow Ctl {}", GIT_VERSION);

    let args = ZFCtl::from_args();
    log::debug!("Args: {:?}", args);

    let zsession = Arc::new(janu::open(janu::config::Config::default()).await.unwrap());
    ...
    match args {
        ZFCtl::Add(ak) => match ak {
            ...
            AddKind::Instance { descriptor_path } => {
                ...
                let df = janu_flow::model::dataflow::descriptor::DataFlowDescriptor::from_yaml(
                    &yaml_df,
                )
                .unwrap();
