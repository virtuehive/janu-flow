...
impl ReadyToken {
    /// Tell Janu Flow to immediately discard the data.
    ///
    ...
    /// Tell Janu Flow to use the data in the "next" run and keep it.
    ///
    ...
    /// (default) Tell Janu Flow to use the data in the "next" run and drop it after.
    ///
    /// The data associated to the `Token` will be transmitted to the `Run` method of the `Operator`
    /// the next time `Run` is called, i.e. the next time the `Input Rule` returns `True`.
