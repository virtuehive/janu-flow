...
use uhlc::HLC;
use janu::Session;
use jnrpc_macros::znservice;
use jrpc::jrpcresult::{JRPCError, JRPCResult};
...
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "lowercase")]
pub enum JanuConfigKind {
    Peer,
    Client,
}

impl std::fmt::Display for JanuConfigKind {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            JanuConfigKind::Peer => write!(f, "peer"),
            JanuConfigKind::Client => write!(f, "client"),
        }
    }
}

impl TryFrom<janu::config::whatami::WhatAmI> for JanuConfigKind {
    type Error = ZFError;
    fn try_from(value: janu::config::whatami::WhatAmI) -> Result<Self, Self::Error> {
        match value {
            janu::config::whatami::WhatAmI::Client => Ok(Self::Client),
            janu::config::whatami::WhatAmI::Peer => Ok(Self::Peer),
...
impl Into<janu::config::whatami::WhatAmI> for JanuConfigKind {
    fn into(self) -> janu::config::whatami::WhatAmI {
        match self {
            Self::Peer => janu::config::whatami::WhatAmI::Peer,
            Self::Client => janu::config::whatami::WhatAmI::Client,
        }
    }
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct JanuConfig {
    pub kind: JanuConfigKind, // whether the runtime is a peer or a client
